...
Pega as informações necessárias, valida população e área, calcula e cria coluna densidade, ordena e pega as 10 primeiras e ultimas linhas.

```{r solucao-218}
  calculaDensidade <- function(pop, area)
    pop/area

  populacaoValida <- function(pop)
    pop > 0

  areaValida <- function(area)
    area > 0
  
  cidadesValidas =  cidades %>%
                    select(cidade, pop_estimada, area, tipo, latitude, longitude) %>%
                    filter(pop_estimada %>% populacaoValida) %>%
                    filter(area %>% areaValida)
  
  cidadesValidas <- cidadesValidas %>% mutate(
    densidade = calculaDensidade(pop_estimada, area)
  )
  
  cidadesOrdenadas =  cidadesValidas %>%
                      arrange(densidade)
  
  maiores = cidadesOrdenadas %>% slice_tail(n = 10)
  maiores %>% select(densidade, cidade, pop_estimada, area, tipo)
  
  menores = cidadesOrdenadas %>% slice_head(n = 10)
  menores %>% select(densidade, cidade, pop_estimada, area, tipo)
```
As 10 cidades com maior densidade demografica são:
...
As 10 cidades com menor densidade demografica são:
...
As cidades de maior densidade demografica são urbanas, e a maioria das cidades de menor densidade demografica são Rurais

É possivel concluir que as cidades urbanas tem maior densidade demográfica.

# 3.4

Pega as informações necessárias, valida IDHM e despesas, plota

```{r solucao-34}
  despesaValida <- function(valor)
    valor > 0

  idhmValido <- function(idhm)
    idhm %>% is.finite
    #idhm > 0

  cidadesValidas <- cidades %>%
                    select(cidade, despesas_municipais, idhm) %>%
                    filter(despesas_municipais %>% despesaValida) %>%
                    filter(idhm %>% idhmValido)

  #cidadesValidas %>%  ggplot(aes(x = despesas_municipais, y = idhm)) +
  #                    geom_point(alpha = 0.05) +
  #                    geom_smooth(aes(group = 1), method = 'lm', se = FALSE)
  
  cidadesValidas %>%  ggplot(aes(x = despesas_municipais, y = idhm)) +
                      geom_point(alpha = 0.05) +
                      scale_x_continuous(limits = c(0, 10e8)) +
                      geom_smooth(aes(group = 1), method = 'lm', se = FALSE)+
                      labs(
  ...
  #cidadesValidas %>% ggplot(aes(x = despesas_municipais, y = idhm)) + geom_jitter(width = 10e9, shape = ".")
```

É possivel concluir que quanto maior a despesa municipal, maior o IDHM. Porém, a cidade pode ter um IDHM alto com despesa municipal relativamente baixa.

# 3.40

Junta colunas de populações, transforma em um fator ordenado, soma as populações, plota.

```{r solucao-340}
  faixaEtariaPorTipo <- cidades %>% pivot_longer(
    cols = starts_with('pop_regular_'),
    names_to = 'faixaEtaria',
    names_prefix = 'pop_regular',
    values_to = 'popTipo'
  ) %>% mutate(
    faixaEtaria = factor(
      faixaEtaria,
      levels = unique(faixaEtaria),
      ordered = TRUE
    )
  )
  
  popTotal = faixaEtariaPorTipo %>% select(pop_regular) %>% sum()

  faixaEtariaPorTipo %>% filter(tipo %>% validaTipo) %>%
                         ggplot(aes(x = tipo, y = popTipo/popTotal, fill=faixaEtaria)) +
...
Cidades urbanas tem proporcionalmente maior população adulta.

# 4.1 - As 10 cidades com menor densidade

Prepara conteudo para o popup, adiciona marcadores

```{r solucao-41}
  classificacao = c(1:10)
  densidade     = menores %>% pull(densidade)
  conteudo      = paste(classificacao, "º densidade populacional: ", densidade, sep = "")
  
  leaflet(menores) %>%
  addTiles() %>% 
  addMarkers(~longitude, ~latitude, popup = ~conteudo, label = ~cidade)
```

## Cidades urbanas com mais tratores

Pega as informações necessárias, filtra, ordena, prepara conteudo do popup e adiciona os marcadores.

```{r} 
  #meses = c("Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro")
  #regex = paste("[\\s]", meses, "[\\s]", sep = "")
  
  #contemMes <- function(cidade){
    #cidade %>% str_detect(meses)
    
    #for(mes in meses){
    #  retorno <- cidade %>% str_detect(mes)
    #}
    #retorno
    ...
    #No soco
    #ifelse(str_detect(cidade, "Janeiro"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Fevereiro"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Marco"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Abril"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Maio"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Junho"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Julho"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Agosto"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Setembro"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Dezembro"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Outubro"), return(TRUE), return(FALSE))
    #ifelse(str_detect(cidade, "Novembro"), return(TRUE), return(FALSE))
  #}
  
  #cidadesComMes = cidades %>% filter(cidade %>% contemMes)
  #(cidadesComMes)
  
  #leaflet(cidadesComMes) %>%
  #addTiles() %>% 
  #addMarkers(~longitude, ~latitude, label = ~cidade)
  
  cidadesUrbanasComMaisTratores = cidades %>%
                                  select(cidade, tratores, tipo, longitude, latitude) %>%
                                  filter(tipo == "Urbano") %>%
                                  arrange(tratores %>% desc) %>%
                                  slice_head(n = 10)
  
  qtdTratores = cidadesUrbanasComMaisTratores %>% pull(tratores)
  
  conteudo = paste(classificacao, "º - ", qtdTratores, " tratores", sep = "")
  
  leaflet(cidadesUrbanasComMaisTratores) %>%
  addTiles() %>% 
  addMarkers(~longitude, ~latitude, label = ~cidade, popup = ~conteudo)
```
